    ...
    def learning(Mdl, xte):
        ...
        if Mdl.loss == '0-1':
            ...
            objective = cvx.Minimize(-Mdl.tau_ @ mu_ \
                                     + cvx.sum(
                np.ones(t) + cvx.max(cvx.reshape(M @ mu_, (2 ** Mdl.labels - 1, t)) - v, axis=0)) / t \
        ...
        if Mdl.deterministic == False:
            ...
            if Mdl.loss == 'log':
                for i in range(t):
                    for j in range(Mdl.labels):
                        Mdl.h[j, i] = 1 / sum(
                            np.exp(Mdl.alpha_[i] * phi(Mdl, xte[i, :], np.arange(1, Mdl.labels + 1)) @ Mdl.mu_ \
